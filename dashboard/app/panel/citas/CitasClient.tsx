...
const ESTADO_CONFIG: Record<string, { label: string; bg: string; color: string }> = {
  ...
  no_asistio: { label: "No asistio", bg: "#fef3c7", color: "#92400e" },
...
type SyncStatus = "success" | "error" | null;
...
export default function CitasClient({ citas, fechaLabel, clinicId, tieneGcal }: CitasClientProps) {
  ...
  const syncNow = async () => {
    ...
    try {
      ...
      if (!res.ok) {
        const detail = data?.detail || "No se pudo sincronizar.";
        setSyncMsg(detail);
        setSyncStatus("error");
        return;
      }
      ...
      setSyncMsg(`Sync completada: ${importados} importadas, ${actualizados} actualizadas.`);
      setSyncStatus("success");
      router.refresh();
    } catch {
      setSyncMsg("Error de red sincronizando Google Calendar.");
      setSyncStatus("error");
    } finally {
      setIsSyncing(false);
    }
  ...
  return (
    <div>
          ...
          <div style={{ display: "flex", flexDirection: "column", alignItems: "flex-end", gap: 6 }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
              <span style={{ fontSize: 11.5, color: "#6b7280", display: "flex", alignItems: "center", gap: 5 }}>
                <svg width="12" height="12" viewBox="0 0 12 12" fill="none">
                  <path d="M10 6A4 4 0 1 1 6 2M6 2l1.5-1.5M6 2L4.5.5" stroke="#2563eb" strokeWidth="1.4" strokeLinecap="round" strokeLinejoin="round" />
                </svg>
                Sync automatica cada 60 min
              </span>
              <button
                onClick={syncNow}
                disabled={isSyncing}
                aria-busy={isSyncing}
                style={{
                  background: isSyncing ? "#e5e7eb" : "#2563eb",
            ...
            {syncMsg && (
              <span
                style={{
                  fontSize: 11.5,
                  color: syncStatus === "error" ? "#b91c1c" : "#166534",
                  fontWeight: 500,
                }}
            ...
            return (
              ...
              >
                <div style={{ display: "flex", alignItems: "center", gap: 14 }}>
                  ...
                  <div>
                    <div style={{ fontSize: 14, fontWeight: 600, color: "#111827" }}>{cita.tipo_servicio || "Cita"}</div>
      ...
      {selected && <ModalDetalleCita cita={selected} onClose={() => setSelected(null)} />}
    </div>
  );
}
    ...
    >
      ...
      >
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 20 }}>
          <div>
            <h3 style={{ margin: "0 0 6px", fontSize: 18, fontWeight: 700, color: "#111827" }}>{cita.tipo_servicio || "Cita"}</h3>
            <span
              style={{
                fontSize: 12,
                fontWeight: 600,
        ...
        <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
          <div style={{ fontSize: 11, fontWeight: 600, color: "#9ca3af", textTransform: "uppercase", letterSpacing: "0.06em" }}>Detalles de la cita</div>
          {start && (
            <>
              <ModalRow
