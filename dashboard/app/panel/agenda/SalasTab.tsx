...
export default function SalasTab({
  ...
  async function save() {
    ...
    try {
      ...
      const url = isNew ? `/api/clinicas/${clinicId}/salas` : `/api/clinicas/${clinicId}/salas/${(editing as Sala).id}`;
      ...
      setEditing(null);
      setNotice({ type: "success", text: "Sala guardada correctamente." });
    } catch (e: any) {
      setError(e.message);
      setNotice({ type: "error", text: "No se pudo guardar la sala." });
    } finally {
      setSaving(false);
    }
  }
  ...
  async function toggleActivo(s: Sala) {
    ...
    if (res.ok) {
      ...
      setNotice({ type: "success", text: `Sala ${updated.activo ? "activada" : "desactivada"}.` });
      return;
    }
    setNotice({ type: "error", text: "No se pudo actualizar el estado de la sala." });
  }

  async function eliminar(s: Sala) {
    if (!confirm(`Eliminar la sala "${s.nombre}"?`)) return;
    setNotice(null);
    const res = await fetch(`/api/clinicas/${clinicId}/salas/${s.id}`, { method: "DELETE" });
    if (res.ok || res.status === 204) {
      setSalas((prev) => prev.filter((x) => x.id !== s.id));
      setNotice({ type: "success", text: "Sala eliminada." });
      return;
    }
    setNotice({ type: "error", text: "No se pudo eliminar la sala." });
  }
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <p style={{ margin: 0, color: "#6b7280", fontSize: 13.5 }}>
          {salas.filter((s) => s.activo).length} salas activas - El agente evita doble reserva de sala automaticamente
        </p>
        ...
        {salas.map((s) => (
          ...
          >
            ...
            <div style={{ display: "flex", gap: 8 }}>
              <button onClick={() => setEditing({ ...s })} style={btnGhostStyle}>
                Editar
              </button>
      ...
      {editing !== null && (
        <div style={overlayStyle} onClick={() => setEditing(null)}>
          <div style={{ ...modalStyle, maxWidth: 420 }} onClick={(e) => e.stopPropagation()}>
            <h3 style={{ margin: "0 0 20px", fontSize: 16, fontWeight: 700 }}>{(editing as Sala).id ? "Editar sala" : "Nueva sala"}</h3>
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <Field label="Nombre *">
                <input
                  ...
                  placeholder="ej: Cabina 1, Box 3, Sala de estetica"
                />
              </Field>
              <Field label="Tipo">
                <select style={inputStyle} value={editing.tipo ?? "sala"} onChange={(e) => setEditing({ ...editing, tipo: e.target.value })}>
                  {TIPOS.map((t) => (
              ...
              <Field label="Capacidad (personas simultaneas)">
                <input
                  type="number"
                  style={inputStyle}
                  value={editing.capacidad ?? 1}
            ...
            <div style={{ display: "flex", gap: 10, marginTop: 20, justifyContent: "flex-end" }}>
              <button onClick={() => setEditing(null)} style={btnGhostStyle}>
                Cancelar
              </button>
