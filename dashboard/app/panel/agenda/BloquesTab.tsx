...
const TIPOS: { value: string; label: string; color: string }[] = [
  ...
  { value: "formacion", label: "Formacion", color: "#0891b2" },
...
export default function BloquesTab({
  ...
  async function crear() {
    ...
    try {
      ...
      setCreating(false);
      setForm(EMPTY_NEW());
      setNotice({ type: "success", text: "Bloqueo creado correctamente." });
    } catch (e: any) {
      setError(e.message);
      setNotice({ type: "error", text: "No se pudo crear el bloqueo." });
    } finally {
      setSaving(false);
    }
  }
  ...
  async function eliminar(b: Bloque) {
    ...
    setNotice({ type: "error", text: "No se pudo eliminar el bloqueo." });
  }
    ...
    <div>
      <div style={{ display: "flex", justifyContent: "space-between", alignItems: "center", marginBottom: 20 }}>
        <p style={{ margin: 0, color: "#6b7280", fontSize: 13.5 }}>
          {bloques.length} bloqueo{bloques.length !== 1 ? "s" : ""} - El agente no ofrecera huecos durante estos periodos
        </p>
        <button
          onClick={() => {
            setCreating(true);
      ...
      {loadingList && (
        <div style={{ textAlign: "center", padding: "32px 0", color: "#9ca3af", fontSize: 13.5 }}>Cargando bloqueos...</div>
      )}
      ...
      <div style={{ display: "flex", flexDirection: "column", gap: 10 }}>
        {sorted.map((b) => {
          ...
          return (
            <div key={b.id} style={{ background: "white", border: "1px solid #e5e7eb", borderRadius: 10, padding: "14px 18px", display: "flex", alignItems: "flex-start", gap: 14 }}>
              <div style={{ width: 8, borderRadius: 4, alignSelf: "stretch", flexShrink: 0, background: meta.color, minHeight: 40 }} />
              <div style={{ flex: 1 }}>
                <div style={{ display: "flex", alignItems: "center", gap: 10, flexWrap: "wrap" }}>
                  <span style={{ fontWeight: 600, fontSize: 14, color: "#111827" }}>{b.titulo}</span>
                  <span style={{ fontSize: 11.5, fontWeight: 600, color: meta.color, background: `${meta.color}18`, padding: "2px 8px", borderRadius: 20 }}>
                    {meta.label}
                  </span>
                </div>
                ...
                {b.notas && <div style={{ fontSize: 12, color: "#9ca3af", marginTop: 2 }}>{b.notas}</div>}
              </div>
        ...
        {!loadingList && bloques.length === 0 && (
          <div style={{ textAlign: "center", padding: "40px 0", color: "#9ca3af", fontSize: 14 }}>
            Sin bloqueos. Crea uno para periodos de vacaciones, formacion o cierres puntuales.
          </div>
        )}
      </div>
        ...
        <div style={overlayStyle} onClick={() => setCreating(false)}>
          <div style={modalStyle} onClick={(e) => e.stopPropagation()}>
            ...
            <div style={{ display: "flex", flexDirection: "column", gap: 14 }}>
              <Field label="Titulo *">
                <input style={inputStyle} value={form.titulo} onChange={(e) => setForm({ ...form, titulo: e.target.value })} placeholder="ej: Vacaciones de verano, Cierre navideno" />
              </Field>
              <Field label="Tipo">
              ...
              <div style={{ display: "grid", gridTemplateColumns: "1fr 1fr", gap: "14px 20px" }}>
                <Field label="Inicio">
                  <input type="datetime-local" style={inputStyle} value={form.fecha_inicio} onChange={(e) => setForm({ ...form, fecha_inicio: e.target.value })} />
                </Field>
                <Field label="Fin">
                  <input type="datetime-local" style={inputStyle} value={form.fecha_fin} onChange={(e) => setForm({ ...form, fecha_fin: e.target.value })} />
                </Field>
              </div>
              {profesionales.length > 0 && (
                <Field label="Profesional afectado (opcional)">
                  <select style={inputStyle} value={form.profesional_id} onChange={(e) => setForm({ ...form, profesional_id: e.target.value })}>
                    <option value="">Toda la clinica</option>
                    {profesionales.map((p) => (
              ...
              {salas.length > 0 && (
                <Field label="Sala afectada (opcional)">
                  <select style={inputStyle} value={form.sala_id} onChange={(e) => setForm({ ...form, sala_id: e.target.value })}>
              ...
              <Field label="Notas">
                <input style={inputStyle} value={form.notas} onChange={(e) => setForm({ ...form, notas: e.target.value })} placeholder="Informacion adicional (opcional)" />
              </Field>
            </div>
            {error && <p style={{ color: "#dc2626", fontSize: 13, marginTop: 12 }}>{error}</p>}
            <div style={{ display: "flex", gap: 10, marginTop: 20, justifyContent: "flex-end" }}>
              <button onClick={() => setCreating(false)} style={btnGhostStyle}>
                Cancelar
              </button>
