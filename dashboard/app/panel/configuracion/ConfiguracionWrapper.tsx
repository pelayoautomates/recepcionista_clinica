...
function buildPrompt(nombre: string, doc: string, tono: Tono): string {
  const tonoTexto = {
    profesional: "Usa un tono profesional y directo. Sé eficiente y claro.",
    cercano: "Usa un tono cercano y amigable. Tutea al paciente y muestra empatía.",
    formal: "Usa un tono formal y clínico. Trata de usted al paciente en todo momento.",
  }[tono];
  return `Eres la recepcionista virtual de ${nombre}. Responde siempre en español.
...
const TONOS: { key: Tono; label: string; desc: string }[] = [
  { key: "profesional", label: "Profesional", desc: "Directo y eficiente. Ideal para clínicas de especialistas." },
  { key: "cercano",     label: "Cercano",      desc: "Amigable y empático. Tutea al paciente. Ideal para medicina general." },
  { key: "formal",      label: "Formal",       desc: "Clínico y de usted. Ideal para hospitales y clínicas de alta gama." },
];
...
const ROUTING_MODES: { key: RoutingMode; label: string; desc: string; badge?: string }[] = [
  ...
  { key: "fuera_horario",   label: "Solo fuera de horario",  desc: "El agente solo contesta cuando la clínica está cerrada.", badge: "Gestionado por nosotros" },
  { key: "si_no_contestan", label: "Solo si no contestamos", desc: "La llamada va primero a recepción; si no contesta nadie, pasa al agente.", badge: "Gestionado por nosotros" },
];
...
export default function ConfiguracionWrapper({ clinica, clinicId }: Props) {
  ...
  async function generarConIA() {
    if (!url.trim() && archivos.length === 0) { setError("Pega la URL de tu clínica o sube un documento."); return; }
    ...
    try {
      ...
      const lines: string[] = [];
      if (clinica.nombre) lines.push(`Clínica: ${clinica.nombre}`);
      ...
      setShowIA(false);
    } catch (e: any) { setError(e.message || "Error generando configuración"); }
  ...
  async function sendChat() {
    ...
    try {
      ...
      setChatError("Error al conectar. ¿Está el backend activo?");
      ...
      <div style={{ maxWidth: 720, paddingBottom: 80 }}>
        ...
        <div style={{ display: "flex", justifyContent: "space-between", alignItems: "flex-start", marginBottom: 24 }}>
          <div>
            ...
            <p style={{ margin: 0, fontSize: 13.5, color: "#6b7280" }}>
              Rellena la información de tu clínica y el agente sabrá todo lo necesario para atender a tus pacientes.
            </p>
          </div>
          <button
          ...
          {showIA && (
            <div style={cardBody}>
              <p style={{ ...hint, marginTop: 0, marginBottom: 12, color: "#6b7280" }}>
                Pega la web de tu clínica y la IA extrae servicios, horarios y precios automáticamente.
              </p>
              <label style={label}>Web de la clínica</label>
                ...
                <div style={{ display: "flex", flexWrap: "wrap", gap: 6, marginBottom: 12 }}>
                  {archivos.map((f, i) => (
                    <span key={i} style={{ background: "#f3f4f6", borderRadius: 20, padding: "3px 10px", fontSize: 12, display: "flex", alignItems: "center", gap: 6 }}>
                      {f.name}
                      <button onClick={() => setArchivos(p => p.filter((_, j) => j !== i))} style={{ background: "none", border: "none", cursor: "pointer", color: "#9ca3af", fontSize: 14, padding: 0 }}>×</button>
                    </span>
                  ))}
                </div>
              )}
        ...
        <div style={card}>
          ...
          <div style={cardBody}>
            ...
            />
            <p style={hint}>Edita libremente. El agente usará exactamente esta información para responder.</p>
          </div>
        </div>
          ...
          <div style={{ ...cardBody, display: "flex", flexDirection: "column", gap: 8 }}>
            ...
            {routing !== "siempre" && (
              <div style={{ background: "#fef9c3", border: "1px solid #fde68a", borderRadius: 8, padding: "10px 14px", fontSize: 12.5, color: "#92400e" }}>
                Este modo requiere configuración en la central telefónica. Te contactaremos para aplicarlo en Telnyx.
              </div>
            )}
          </div>
        </div>
        ...
        <div style={card}>
          ...
          <div style={cardBody}>
            <input
              ...
              style={inputSt}
            />
            <p style={hint}>Recibirás un aviso cuando el agente agende una cita o escale a humano.</p>
          </div>
        </div>
        ...
        <div style={{ flex: 1, overflowY: "auto", padding: 16, display: "flex", flexDirection: "column", gap: 10 }}>
          {messages.length === 0 && (
            <div style={{ textAlign: "center", color: "#9ca3af", fontSize: 13, marginTop: 32 }}>
              Escribe un mensaje para probar cómo responde el agente con la configuración actual.
            </div>
          )}
          {messages.map((m, i) => (
